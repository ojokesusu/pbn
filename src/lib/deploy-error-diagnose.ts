...
const PATTERNS: Array<{
  ...
  {
    match: /Timeout \(control socket\)|Timeout \(data socket\)|Timeout when trying to open data connection/i,
    build: () => ({
      category: "FTP_PASV_TIMEOUT",
      label: "FTP PASV / control-socket timeout",
      cause: "basic-ftp control / data socket idle-timeout — biasanya passive-mode handshake stuck di NAT/firewall path (Rumahweb shared hosting kasih PASV IP backend yang beda dari control host, atau worker outbound diblok ke high-port range).",
      fix: "Auto-retry udah jalan (90s timeout + allowSeparateTransferHost). Kalau masih persisten di host yang sama, whitelist PASV port range (biasanya 49152–65534) di outbound firewall RDP/worker, atau pindah domain ke server lain.",
      severity: "transient",
      actionHref: "/servers",
      actionLabel: "Cek /servers",
    }),
  },
  {
    match: /^\[NET\] /i,
    build: () => ({
      category: "FTP_NET_TAGGED",
      label: "FTP gagal (net/timeout — sudah di-retry)",
      cause: "PASV/NAT path stuck atau koneksi drop. Sudah retry sekali dengan backoff 2.5s, masih gagal.",
      fix: "Suspect outbound firewall RDP/worker ke PASV port range. Kalau seluruh batch satu server, server itu down — cek /health-check.",
      severity: "transient",
  ...
  },
  {
    match: /^\[AUTH\] /i,
    build: () => ({
      category: "FTP_AUTH_TAGGED",
      label: "FTP login ditolak (post-retry)",
      cause: "530 Login authentication failed — sudah retry sekali setelah 6s sleep buat ride-out cPanel PAM propagation. Masih ditolak berarti creds beneran salah atau FTP user belum exist.",
      fix: "Buka /servers, re-test FTP creds untuk host ini. Kalau salah, re-create FTP user di cPanel + update DB. Kalau benar, mungkin cPanel butuh propagasi >6s — kasih jeda lebih lama di scheduler antara AddSite dan deploy.",
      severity: "config",
      actionHref: "/servers",
      actionLabel: "Cek /servers",
    }),
  },
  {
    match: /^\[TLS\] /i,
    build: () => ({
      category: "FTP_TLS_TAGGED",
      label: "FTPS / TLS handshake gagal",
      cause: "TLS negotiation gagal — cert expired, server gak support TLS, atau cipher mismatch.",
      fix: "Cek cert FTP server. Kalau Pure-FTPd, restart service. Kalau persisten, downgrade ke plain FTP di config server.",
      severity: "config",
      actionHref: "/servers",
      actionLabel: "Cek /servers",
    }),
