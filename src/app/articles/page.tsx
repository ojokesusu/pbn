...
export default function ArticlesPage() {
  ...
  const filtered = articles.filter((a) => {
    ...
    return a.title.toLowerCase().includes(q) || a.domain?.name?.toLowerCase().includes(q) || a.category?.name?.toLowerCase().includes(q) || a.authorName?.toLowerCase().includes(q)
  })
  const totalPages = Math.ceil(filtered.length / perPage)
  const paginated = filtered.slice((currentPage - 1) * perPage, currentPage * perPage)

  const fetchArticles = useCallback(async (domainId?: string) => {
    setLoading(true)
    try {
      const url = domainId
        ? `/api/articles?domainId=${domainId}`
        : "/api/articles"
  ...
  useEffect(() => {
    fetchArticles(selectedDomainId || undefined)
  }, [selectedDomainId, fetchArticles])
