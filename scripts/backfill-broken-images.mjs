// Backfill the 2277 articles that ended up with image.pollinations.ai URLs
// when the AI image service turned paid (2026-06-06, HTTP 402). For each
// affected article: query Unsplash with the title, fall back to Pexels if
// Unsplash misses, store the new URL on Article.featuredImage. Articles that
// even Pexels can't satisfy get featuredImage = null so the EJS template
// stops rendering a broken <img> tag.
//
// Rate limits we respect:
//   Unsplash demo tier:  50 req/hour per key  → ~1.2s between calls is safe
//   Pexels demo tier:   200 req/hour per key  → only used as fallback so far
//                                                under cap
//
// Usage:
...
async function main() {
  ...
  for (let i = 0; i < target.length; i++) {
    ...
    if (newUrl) {
      ...
      // No source available — null it so the broken <img> stops rendering.
      pending.push({ id: art.id, url: null });
      nulled++;
      console.log(`  [${i + 1}/${target.length}] ${art.domain_name} :: ${art.title.substring(0, 50)} -> NULL (no source)`);
    }
    ...
    if (pending.length >= BATCH || i === target.length - 1) {
      if (!DRY) {
        ...
        try {
          ...
          console.log(`    -- flushed batch of ${pending.length}, total written: ${written}, nulled: ${nulled}`);
        } catch (e) {
  ...
  console.log(`\nDONE: ${written} updated with new image URL, ${nulled} nulled out, ${skipped} skipped`);
  await client.end();
}
